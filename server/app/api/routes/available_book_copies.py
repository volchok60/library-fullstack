...
router = APIRouter(prefix="/copies/available", tags=["available-copies"])
...
def read_book_copies(session: SessionDep, skip: int = 0, limit: int = 100) -> BookCopiesPublic:
    ...
    count_statement = select(func.count()).select_from(BookCopy) #TODO: Adjust to filter available copies
    count = session.exec(count_statement).one()
    # Fetch all books
