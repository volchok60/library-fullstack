...
export default function Genres() {
  ...
  const getGenreIcon = (genreName: string) => {
    ...
    if (name.includes('fantasy')) return '🧙‍♂️'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Genres</h1>
              <p className="text-gray-600">
                Explore different categories and literary styles in our collection
              </p>
            </div>
            <Link 
              to="/genres/create" 
              className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-600 text-white font-semibold rounded-xl shadow-lg hover:from-purple-600 hover:to-pink-700 transform hover:scale-105 transition-all duration-200"
            >
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div className="flex items-center space-x-6">
              <div className="text-center">
                ...
                <div className="text-sm text-gray-500">Total Genres</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-pink-600">{filteredGenres.length}</div>
        ...
        {filteredGenres.length === 0 ? (
          <div className="text-center py-12">
            <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md mx-auto">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                {searchTerm ? 'No genres found' : 'No genres yet'}
              </h3>
              <p className="text-gray-600 mb-6">
                {searchTerm 
                  ? `No genres match "${searchTerm}". Try a different search term.`
                  : 'Start organizing your library by adding your first genre category.'
                }
              </p>
              {!searchTerm && (
                <Link 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredGenres.map((genre: GenreType, index) => (
              <Link
                key={genre.id}
                to={`/genres/${genre.id}`}
              ...
              >
                <div className="p-6">
                  ...
                  <div className="text-center">
                    ...
                    {/* Action Indicator */}
                    <div className="flex items-center justify-center text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      <span className="text-sm font-medium mr-1">Explore Genre</span>
                ...
                {/* Hover Effect Border */}
                <div className={`h-1 bg-gradient-to-r ${getGenreColor(index)} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left`}></div>
              </Link>
            ))}
          </div>
        )}

        {/* Popular Genres Showcase */}
        {genres.length > 0 && (
          <div className="mt-12 bg-white rounded-2xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Genre Categories</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {filteredGenres.slice(0, 12).map((genre, index) => (
                <div
          ...
          <div className="mt-8 bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                ...
                <span className="font-medium">Add Genre</span>
              </Link>
              
              <Link
                to="/books/create"
                className="flex items-center p-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all duration-200 transform hover:scale-105"
              >
                <svg className="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              >
                ...
                <span className="font-medium">View Authors</span>
              </Link>
            </div>
          </div>
        )}
